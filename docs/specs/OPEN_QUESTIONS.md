...
| 2026-09-13 | owner decision (income lens on the Options tab) | contract_analytics gains the two comparable income numbers for the selected contract: income on cash posted (premium ÷ strike) and income per unit of assignment risk (cash % ÷ \|delta\|, omitted without a delta estimate); from 21 DTE also annualized cash % and its ratio to the 10Y (rate param). The CSP row leads with these two; cushion, \|delta\| and breakeven support; pay_vs_cushion dropped from the UI ($ per 1% of spot is not cross-ticker). Caption states comparability holds at the same expiry only, and the annualized form exists from 21 DTE. Options remain display-only — no ranking, no hiding, `_highlighted` untouched |
